function [zR, aR] = vac_path_now(dn, sf, sz, R, dz)
% beam path calculation by ABCD ray transfer matrix for 2015 campaign
...
fprintf('Calculate vacuum ECE path AFTER 2015 campaign \n');
fprintf('device number : %d, focus lens position : %d [mm], zoom lens position : %d [mm] \n', dn, sf, sz);

% will find an beam angle at R = 2.39 m
R = R*1000; % [m] -> [mm]
% for R = [1600:50:2300]
if dn == 1 % LFS
    sp = 3350 - R; 
...
% vertical position from the reference axis (vertical center of all lens, z=0+dzline) at ECEI array box
zECEI = ((24:-1:1) - 12.5)*14 + dz; % [mm]
